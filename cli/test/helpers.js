...
function parseArrayHeaderLine(content, defaultDelimiter) {
    ...
    if (braceStart !== -1 && braceStart < colonIndex) {
        ...
        if (foundBraceEnd !== -1 && foundBraceEnd < colonIndex)
            fields = parseDelimitedValues(content.slice(braceStart + 1, foundBraceEnd), delimiter).map(field =>
...
function* decodeStreamSync$1(source, options) {
    ...
    const cursor = new StreamingLineCursor(
        parseLinesSync(source, resolvedOptions.indent, resolvedOptions.strict, scanState),
        scanState
    );
...
async function* decodeStream$1(source, options) {
    ...
    if (Symbol.asyncIterator in source) {
        const cursor = new StreamingLineCursor(
            parseLinesAsync(source, resolvedOptions.indent, resolvedOptions.strict, scanState),
            scanState
        );
...
function normalizeValue(value) {
    ...
    if (isPlainObject(value)) {
        ...
        for (const key in value)
            if (Object.prototype.hasOwnProperty.call(value, key)) normalized[key] = normalizeValue(value[key]);
        return normalized;
    }
    return null;
}
...
function* encodeObjectAsListItemLines(obj, depth, options) {
    ...
    if (isJsonPrimitive(firstValue))
        ...
        if (firstValue.length === 0)
            yield indentedListItem(
                depth,
                `${encodedKey}${formatHeader(0, {delimiter: options.delimiter})}`,
                options.indent
            );
